...
setwd("/share/milklab/proteomics/output_Process_GPM/") # destination path
data_path   = "/share/milklab/proteomics/output_XTandem/test_process_gpm/" # includes bio-matched reference databases
...
input_db_matched  = system(paste("grep -m 1 '<file type=\"peptide\"' ", data_path, "*", sep=""), intern=TRUE)
input_output_matched = system(paste("grep '<bioml' ", data_path, "*", sep=""), intern =TRUE)
# This prefix is in the header and exists uniquely in each file

# Process xml file excerpts for file and database info
#current_loc = str_extract(input_out_matched, ".*GPM[0-9]+.xml") # this is only needed if working with tandem output from proteomics core
# current_loc = str_extract(input_out_matched, ".*GPM[0-9]+.xml") # file path to dxtandem output, this is redundant with file list

fasta_desc  = str_extract(str_extract(input_db_matched, "URL=.+.fasta"), "[A-Za-z0-9-_+]+.fasta$") # database used in peptide to protein process
...
# bio_sample_mm = str_extract(input_out_matched, "MMU[0-9]+")
bio_sample = str_extract(input_output_matched, "HSA[0-9]+")
# bio_sample_unmatched = str_extract(input_out_matched, "GPM[0-9]+.mgf") # this needs to be completed for the unmatched spectra because scaffold renames the input file


# gpm_moving  = as.data.frame(cbind(current_loc, bio_sample, bio_sample_unmatched, fasta_desc)) # database with both
run_info  = as.data.frame(cbind(output_gpm, bio_sample, fasta_desc)) # database with both
...
spec_assigned  = system(paste("grep -m 1 '<note label=\"modelling, total spectra assigned\"' ", data_path, "*", sep=""), intern=TRUE)
spec_assigned  = as.numeric(str_extract(spec_assigned, "[0-9]+"))

spec_used  = system(paste("grep -m 1 '<note label=\"modelling, total spectra used\"' ", data_path, "*", sep=""), intern=TRUE)
spec_used  = as.numeric(str_extract(spec_used, "[0-9]+")) # total spectra per xml file is same regardless of database

unique_assigned  = system(paste("grep -m 1 '<note label=\"modelling, total unique assigned\"' ", data_path, "*", sep=""), intern=TRUE)
unique_assigned  = as.numeric(str_extract(unique_assigned, "[0-9]+"))

percent_assigned = spec_assigned/spec_used*100

run_stats= cbind(run_info, db_size, spec_assigned, spec_used, unique_assigned, percent_assigned)

write.table(run_stats, file="./test_proc_gpm_stats.tsv", sep="\t", quote=FALSE, row.names=FALSE)
...
# # # # # # # # #
#
# TODO if working with data from Proteomics Core
#
# # # # # # # # #
